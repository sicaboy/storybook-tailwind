...
<template>
  <div class="bg-white">
    ...
    >
      ...
      <div
        class="mt-16 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 sm:gap-y-16 lg:grid-cols-3 lg:gap-x-8 xl:grid-cols-4"
      >
        <div v-for="order in orders" :key="order.id" class="group relative">
          <div
            class="aspect-w-1 aspect-h-1 bg-gray-200 rounded-md overflow-hidden group-hover:opacity-75"
          >
            <img
              :src="order.imageSrc"
              :alt="order.imageAlt"
          ...
          <p class="mt-1 text-lg font-medium">
            ...
            >
            <span v-else-if="order.status === 'cancelled'" class="text-gray-500"
              >Cancelled</span
            >
          </p>
        </div>
